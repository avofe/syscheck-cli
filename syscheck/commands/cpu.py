...
def show_cpu() -> None:
    ...
    colour = get_color_for_percent(data["percent"], "cpu")
    console.print(
        f"load        [{colour}]{data['percent']}%[/] {make_bar(data['percent'], name='cpu')}"
    )
    ...
    if per_cpu:
        parts = "  ".join(
            f"[{get_color_for_percent(p, 'cpu')}]{i}: {p:.0f}[/]" for i, p in enumerate(per_cpu)
        )
